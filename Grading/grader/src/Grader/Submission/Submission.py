...
class Submission:
    ...
    def get_student_id(self) -> str:
        ...
        if len(zip_files) == 1:
            self.valid = True
            if "_" in zip_files[0]:
                return zip_files[0].split("_")[1].replace(".zip", "")
    ...
    def organize(self):
        ...
        for file in files:
            ...
            if file.endswith("Main.java"):
                entry_point_file = file
                break
            if file.endswith("Project4.java"):
                entry_point_file = file
                break
            if file.endswith("main.java"):
                entry_point_file = file
    ...
    def compile(self):
        ...
        self.valid = os.path.exists(os.path.join(grading_dir, "bin", self.entry_point.replace(".java", ".class"))) or os.path.exists(os.path.join(grading_dir, "bin", "Main.class")) or os.path.exists(os.path.join(grading_dir, "bin", "main.class")) or os.path.exists(os.path.join(grading_dir, "bin", "Project4.class"))
        os.chdir(original_dir)
        return

    def run(self, input_path: str, timeout: int = 5):
        case_name = os.path.basename(input_path).replace(".in", "")
        inp_no = int(input_path.split("/")[6][3])
        if "10" in input_path.split("/")[6]:
            inp_no = 10
        if inp_no < 8:
            timeout = 35
        elif inp_no == 8:
        ...
        try:
            ...
            try:
                ...
                try:
                    subprocess.run(
                ["java", "Project4", input_path, output_path],
                stdout=PIPE, stderr=PIPE, timeout=timeout, check=True)
                except subprocess.CalledProcessError as failed:
                    try:
                        subprocess.run(
    ...
    def compiled(self) -> bool:
        if not self.valid:
            return False
        grading_dir = os.path.join(self.submission_path, "grading")
        return os.path.exists(os.path.join(grading_dir, "bin", self.entry_point.replace(".java", ".class"))) or os.path.exists(os.path.join(grading_dir, "bin", "Main.class")) or os.path.exists(os.path.join(grading_dir, "bin", "main.class")) or os.path.exists(os.path.join(grading_dir, "bin", "Project4.class"))
